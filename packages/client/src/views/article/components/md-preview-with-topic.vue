  ...
  <div class="container">
    <ul class="toc left-toc">
      <li
        v-for="(t, i) in toc"
        :key="i"
        :style="{ paddingLeft: t.indent * 12 + 'px' }"
...
const buildToc = async () => {
  ...
  const tags = Array.from(
    new Set(Array.from(anchors).map((e: any) => e.tagName)),
  ).sort();
...
const scrollTo = async (anchor: any) => {
  await nextTick();
  const el = preview.value.$el.querySelector(
...
const onScroll = (e: any) => {
  ...
  for (let i = headings.length - 1; i >= 0; i--) {
    ...
    if (scrollTop >= top - 20) {
      activeIndex.value = toc.value.findIndex(
        t => t.lineIndex === headings[i].getAttribute('data-v-md-line'),
      );
      break;
    }
  }
