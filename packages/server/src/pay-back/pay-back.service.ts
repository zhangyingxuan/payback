...
import { params, baseUrl } from './utils/config';
import * as dayjs from 'dayjs';
import { Cron } from '@nestjs/schedule';

@Injectable()
export class PayBackService {
  constructor(@InjectRepository(shortTermData) private readonly shortTermDataRp: Repository<shortTermData>) { }
  ...
  async crawlTodayData() {
    this.logger.debug('Called is Begining!');
    // 如果存在数据，则返回已有该数据
    ...
    // 准备涨停数据
    const dailyLimitData: Object[] = await playWrightUtil.prepareDataByUrl(baseUrl + params.dailyLimitMoreThan1);
    // 跌停数据
    const downLimitData: Object[] = await playWrightUtil.prepareDataByUrl(baseUrl + params.downLimit);
    ...
    createPayBackDto.marketHeight = dailyLimitData[0][`连续涨停天数[${currentDate}]`];

    let SZAmount = 0, SHAmount = 0;
