...
export class systemConfig {
  ...
  @Column({ comment: '每日收盘是否自动添加自选个股 0不添加 1添加', type: 'tinyint', width: 1 })
  isAutoAddSelf: boolean;

  @Column({ comment: '自选连板 0不添加 1添加', type: 'tinyint', width: 1 })
  isAutoAddSelfEvenBoard: boolean;

  @Column({ comment: '自选首板 0不添加 1添加', type: 'tinyint', width: 1 })
  ...
  @Column({ comment: '是否竞价删除 连板', type: 'tinyint', width: 1 })
