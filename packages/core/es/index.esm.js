...
function dailyLimitOptionalStrategy(stock, currentLevel) {
    ...
    if (stock.code.startsWith('3') || stock.code.startsWith('688') || stock.code.startsWith('83'))
        return false;
    // 连板全部加入
    if (currentLevel != 1)
        return true;
...
const chooseStockBaseCondition = '行业；同花顺二级行业；股价低于30元；流通市值<=120亿；流通市值>=20亿；非创业板；非科创板；非ST';
const params = {
    downLimit: '跌停；非st；行业；同花顺二级行业',
    downLimitByDate: '${date}跌停；非st；行业；同花顺二级行业',
    // 炸板个股；涨停未遂
    dailyLimitOpen: '涨停打开；非ST；成交额；行业；同花顺二级行业；',
    dailyLimitOpenByDate: '${date}涨停打开；非ST；成交额；行业；同花顺二级行业；',
    hugeFall: '跌幅大于等于15的个股；行业；同花顺二级行业；',
    hugeFallByDate: '${date}跌幅大于等于15的个股；行业；同花顺二级行业；',
    dailyLimitMoreThan1: '涨停；非ST；几天几板；涨停原因；封板金额；成交额；换手率；行业；同花顺二级行业；',
    dailyLimitMoreThan1ByDate: '${date}涨停；非ST；几天几板；涨停原因；封板金额；成交额；换手率；行业；同花顺二级行业；',
    // 获取昨日涨停的数据（客观数据）
    dailyLimitYesterday: '昨日涨停；开板次数；首次涨停时间；最终涨停时间；几天几板；昨日竞价量情况；今日竞价量情况；集合竞价评级；竞价涨幅；竞价异动类型；非ST；行业；同花顺二级行业；',
    // =============== =============== 选股 start  =============== ===============
    ...
    // 新股
    chooseStockNewStock: '今日新股上市；行业；同花顺二级行业；竞价涨幅；流通市值；',
    // =============== =============== 选股 end  =============== ===============
    // 近三日资金流向
